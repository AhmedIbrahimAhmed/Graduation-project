...
const getAllStadiumsService = async () => {
    const res = await models_1.Stadium.findAndCountAll({
...
const UpdateStadiumGalleryService = async (req) => {
    ...
    const avater = await models_1.Gallery.findOne({
        where: { StadiumId },
        attributes: ['image'],
    });
    await models_1.Stadium.update({ image: avater?.dataValues.image }, { where: { id: StadiumId } });
    return {
        status: 200,
