...
export const getMyMatchesService = async (
  ...
  if (req.user?.role === 'stadium') {
    const myMatche = await Match.findAll({
      where: {
        stadiumId: userId,
      },
    });
...
export const deletePlayrerMatch = async (
  req: Request,
): Promise<IServiceResponse> => {
  const { matchId } = req.params;
  const userId = req.user?.id;
  ...
  if (!match) {
    throw new CustomError(404, 'هذا الدوري غير موجود');
  }

  const checkeUserIsPlayer = await MatchPlayer.findOne({
    where: { matchId, userId },
  });

  if (!checkeUserIsPlayer) {
    throw new CustomError(401, 'لا يمكنك حذف هذا الدوري');
  }
...
export const delOwnMatch = async (req: Request): Promise<IServiceResponse> => {
  const { matchId } = req.params;
  const userId = req.user?.id;
  ...
  const checkeUserIsOwner = await Match.findOne({
  ...
  const ownMatch = await Match.findAll({
