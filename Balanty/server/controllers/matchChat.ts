...
const getAllUserMessages: RequestHandler = async (
  req: Request,
  res: Response,
): Promise<void> => {
  const { id } = req.params;
  const data = await getAllUserMessagesService(+id, req);

  res.status(data.status).json(data);
};

const getAllUserChats: RequestHandler = async (
  req: Request,
  res: Response,
): Promise<void> => {
  const { id } = req.params;

  const data = await getAllUserChatsService(+id, req);
